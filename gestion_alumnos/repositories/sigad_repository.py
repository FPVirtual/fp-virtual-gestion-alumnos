...
class SIGADRepository:
    ...
    def buscar_por_documento(self, documento: str) -> Registro | None:
    ...
    def _obtener_estudiantes_con_reintentos(self, id_solicitud: int) -> dict:
        ...
        for intento in range(1, max_retries + 1):
            try:
                logger.debug(f"Intento {intento}/{max_retries}: descargando estudiantes")
                ...
                if intento == max_retries:
                    raise APITimeoutError(f"Timeout después de {max_retries} intentos")
                time.sleep(delay)
            except requests.RequestException as e:
                raise APIError(f"Error de conexión: {e}")
        
        raise APIError("No se pudo obtener los datos después de todos los reintentos")
    ...
    def _guardar_copia_local(self, datos: dict, id_solicitud: int) -> None:
        ...
        # En producción, limpiar archivos antiguos
